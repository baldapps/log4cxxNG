...
public:
	...
	{
		MessageBuffer buf;
		const log4cxxng::UniChar hello[] = { 'H', 'e', 'l', 'l', 'o', 0 };
		const log4cxxng::UniChar world[] = { ',', ' ', 'W', 'o', 'r', 'l', 'd', 0 };
		const log4cxxng::UniChar greeting[] = { 'H', 'e', 'l', 'l', 'o',
				',', ' ', 'W', 'o', 'r', 'l', 'd', 0
			};
		UniCharMessageBuffer& retval = buf << hello << world;
		LOGUNIT_ASSERT_EQUAL(std::basic_string<log4cxxng::UniChar>(greeting), buf.str(retval));
		LOGUNIT_ASSERT_EQUAL(false, buf.hasStream());
	}
	...
	void testInsertUString()
	{
		...
		UniCharMessageBuffer& retval = buf << std::basic_string<log4cxxng::UniChar>(hello)
			<< std::basic_string<log4cxxng::UniChar>(world);
		LOGUNIT_ASSERT_EQUAL(std::basic_string<log4cxxng::UniChar>(greeting), buf.str(retval));
		LOGUNIT_ASSERT_EQUAL(false, buf.hasStream());
	}
